...
def plot_test(epoch, x_test, generator, figsize=(10,10)):   
    
    generated_images = generator.predict(x_test.flatten().reshape(1,4096))
    
    plt.figure(figsize=figsize)
    plt.imshow(denormalize_image(generated_images[0]), interpolation='nearest')

    plt.tight_layout()
    plt.savefig('figures/yaya_pre_' + str(epoch) + '.png')
